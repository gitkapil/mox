...
Scenario: Validate that a developer hitting merchant APIM gets no role and 'merchant server app id' as aud within claimset in token (sandbox client app doesnot have permission to access merchant server app)
  ...
  And I have valid payment details
  And I make a request for the payment
  And I should receive a "401" error response with "Error validating JWT" error description and "401" errorcode within payment response
  And error message should be "API Gateway Authentication Failed" within payment response
  And I have a payment id "b15e090a-5e97-4b44-a67e-542eb2aa0f4d"
  And I make a request for the check status
  Then I should receive a "401" error response with "Error validating JWT" error description and "401" errorcode within check status response
  And error message should be "API Gateway Authentication Failed" within check status response
...
Scenario: Validate that a merchant with valid token and no paymentRequest as role is not able to access merchant payment request and check status APIs (merchant client app has permission to access merchant server app)
  ...
  And I have valid payment details
  And I make a request for the payment
  And I should receive a "401" error response with "Claim value mismatch: roles=paymentRequest" error description and "401" errorcode within payment response
  And error message should be "TokenClaimValueMismatch" within payment response
  And I have a payment id "b15e090a-5e97-4b44-a67e-542eb2aa0f4d"
  And I make a request for the check status
  Then I should receive a "401" error response with "Claim value mismatch: roles=paymentRequest" error description and "401" errorcode within check status response
...
Scenario: Validate that a developer with valid token and paymentRequest as role is not able to access merchant payment request and check status APIs (sandbox client app doesnot have permission to access merchant server app)
  ...
  And I have valid payment details
  And I make a request for the payment
  And I should receive a "401" error response with "Claim value mismatch: aud=" error description and "401" errorcode within payment response
  And error message should be "TokenClaimValueMismatch" within payment response
  And I have a payment id "b15e090a-5e97-4b44-a67e-542eb2aa0f4d"
  And I make a request for the check status
  Then I should receive a "401" error response with "Claim value mismatch: aud=" error description and "401" errorcode within check status response
  And error message should be "TokenClaimValueMismatch" within check status response
