...
Scenario Outline: Negative flow- Peak error response parsed by DRAGON
  ...
  #And the payment request response should be signed
    Examples:
      ...
      |1000001    |HKD     |/return        |Field error in object 'paymentRequestInputModel': field 'totalAmount' must be less than or equal to 1000000.0; rejected value [1000001.0]                 |Service Request Validation Failed|EA017     |/confirmation     |/unsuccessful  |20               |
      |0.011      |HKD     |/return        |Field error in object 'paymentRequestInputModel': field 'totalAmount' numeric value out of bounds (<7 digits>.<2 digits> expected); rejected value [0.011]|Service Request Validation Failed|EA017     |/confirmation     |/unsuccessful  |20               |
      # DRAG-1258: Effective Duration Business Boundaries
      |150.00     |HKD     |/return        |Field error in object 'paymentRequestInputModel': field 'effectiveDuration' must be greater than or equal to 15; rejected value [14] |Service Request Validation Failed|EA002     |/confirmation     |/unsuccessful  |14               |
      |150.00     |HKD     |/return        |Field error in object 'paymentRequestInputModel': field 'effectiveDuration' must be less than or equal to 3600; rejected value [3601]|Service Request Validation Failed|EA002     |/confirmation     |/unsuccessful  |3601             |
