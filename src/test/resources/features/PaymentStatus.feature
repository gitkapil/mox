...
Scenario Outline: Negative flow- Mandatory fields not sent in the header
 ...
 |Header Accept was not found in the request. Access denied.         | HeaderNotFound|Accept         |400        |
...
Scenario Outline: Negative flow- Invalid PaymentIds sent in the request
  ...
  And I make a request for the payment
  And I should recieve a successful payment response
  And I have an invalid "<payment_id>"
  When I make a request for the check status
 ...
 |Header Payment Request Id was not found in the request. Access denied.                       | Service Request Validation Failed              |                  |EA002      |400|
 |PayMe user scans/imports a Merchant Dynamic/Static PayCode, that is not found in QR database.| Unable to Find Resources During Service Request| random_paymnet_id|EB001      |404|


@trial
Scenario Outline: Positive flow- A merchant is able to create a check status request with all the valid inputs
  Given I am an authorized merchant
  And I have a valid "<payment_id>"
  When I make a request for the check status
  Then I should recieve a successful check status response
  ...
  |1         |Payment Request Expired      |PR007       |
  |2         |Payment Success              |PR005       |
