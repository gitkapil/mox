...
public class PaymentRequest_StepDefs extends UtilManager {
    ...
    public void i_have_merchant_data(String description, String orderId, String additionalData) {
        testContext.getApiManager().getPaymentRequest().setOrderDescription(description);
        testContext.getApiManager().getPaymentRequest().setOrderId(orderId);
    ...
    @When("^I make a request for the payment with POS role using \"([^\"]*)\"$")
    public void i_make_a_request_for_the_paymentWithPOSRole(String subUnitId) {
        logger.info("********** Creating Payment Request ***********");
        testContext.getApiManager().getPaymentRequest().retrievePaymentRequestWithPOSRole(getRestHelper().getBaseURI() + getFileHelper().getValueFromPropertiesFile(Hooks.generalProperties, "create_payment_request_resource"),
                testContext.getApiManager().getMerchantManagementSigningKeyId(),
                getFileHelper().getValueFromPropertiesFile(Hooks.generalProperties, "signing_algorithm"),
                testContext.getApiManager().getMerchantManagementSigningKey(),
    ...
    public void i_make_a_request_for_the_paymentWithPOSRoleUsingSubUnitId() {
        logger.info("********** Creating Payment Request ***********");
    ...
    @When("^I make a request for the payment with POS role with device id \"([^\"]*)\"$")
    public void i_make_a_request_for_the_paymentWithPOSRoleUsingInvalidDeviceId(String deviceId) {
        logger.info("********** Creating Payment Request ***********");
        String env = PropertyHelper.getInstance().getPropertyCascading("env");
        String usertype = PropertyHelper.getInstance().getPropertyCascading("usertype");
        if (env.equalsIgnoreCase("SIT") && usertype.equalsIgnoreCase("merchant")) {
        ...
        testContext.getApiManager().getPaymentRequest().setDeviceId(deviceId);
        testContext.getApiManager().getPaymentRequest().retrievePaymentRequestWithPOSRole(getRestHelper().getBaseURI() + getFileHelper().getValueFromPropertiesFile(Hooks.generalProperties, "create_payment_request_resource"),
                testContext.getApiManager().getMerchantManagementSigningKeyId(),
                getFileHelper().getValueFromPropertiesFile(Hooks.generalProperties, "signing_algorithm"),
                testContext.getApiManager().getMerchantManagementSigningKey(),
                new HashSet(Arrays.asList(getFileHelper().getValueFromPropertiesFile(Hooks.generalProperties, "header-list-post").split(","))), subUnitId, deviceId);
    }

    @When("^I make a request for the payment with POS role with missing header \"([^\"]*)\"$")
    public void i_make_a_request_for_the_paymentWithPOSRoleUsingSubUnitIdAndMissingHeader(String missingHeader) {
        logger.info("********** Creating Payment Request ***********");
        ...
        testContext.getApiManager().getPaymentRequest().retrievePaymentRequestWithPOSRoleAndMissingHeader(getRestHelper().getBaseURI() + getFileHelper().getValueFromPropertiesFile(Hooks.generalProperties, "create_payment_request_resource"),
                testContext.getApiManager().getMerchantManagementSigningKeyId(),
                getFileHelper().getValueFromPropertiesFile(Hooks.generalProperties, "signing_algorithm"),
                testContext.getApiManager().getMerchantManagementSigningKey(),
    ...
    public void i_should_receive_a_successful_response() {
        ...
        System.out.println("setRequestID: " + testContext.getApiManager().getPaymentRequest().paymentRequestIdInResponse());
        testContext.getApiManager().getPaymentRequest().setPaymentRequestId(testContext.getApiManager().getPaymentRequest().paymentRequestIdInResponse());
    }
