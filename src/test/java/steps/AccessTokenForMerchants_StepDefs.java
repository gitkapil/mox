...
public class AccessTokenForMerchants_StepDefs extends UtilManager{
    ...
    public void i_am_an_userWithPOSRole() {
        try {
            logger.info("********* User Type: " + PropertyHelper.getInstance().getPropertyCascading("usertype") + " ****************");
            ...
            if (PropertyHelper.getInstance().getPropertyCascading("usertype").equalsIgnoreCase("merchant")) {
                ...
                testContext.getApiManager().getAccessToken().setType("merchant");
                testContext.getApiManager().getAccessToken().setMerchantDetails(getFileHelper().getValueFromPropertiesFile(Hooks.envProperties, "merchant-client-id-pos"),
                        getFileHelper().getValueFromPropertiesFile(Hooks.envProperties, "merchant-client-secret-pos"));

                testContext.getApiManager().getAccessToken().setEndpoint(getFileHelper().getValueFromPropertiesFile(Hooks.envProperties, "merchant-api-management-url")
                        +getFileHelper().getValueFromPropertiesFile(Hooks.generalProperties, "Base_Path_Token"));

                testContext.getApiManager().setMerchantManagementSigningKeyId(getFileHelper().getValueFromPropertiesFile(
                        Hooks.envProperties, "merchant-signing-key-id-pos"));

                testContext.getApiManager().setMerchantManagementSigningKey(getFileHelper().getValueFromPropertiesFile(
                        Hooks.envProperties, "merchant-signing-key-pos"));
                ...
                testContext.getApiManager().getAccessToken().setType("sandbox");
                testContext.getApiManager().getAccessToken().setMerchantDetails(getFileHelper().getValueFromPropertiesFile(Hooks.envProperties, "developer-client-id-pos"),
                        getFileHelper().getValueFromPropertiesFile(Hooks.envProperties, "developer-client-secret-pos"));

                testContext.getApiManager().getAccessToken().setEndpoint(getFileHelper().getValueFromPropertiesFile(Hooks.envProperties, "sandbox-api-management-url")
                        +getFileHelper().getValueFromPropertiesFile(Hooks.generalProperties, "Base_Path_Token"));

                testContext.getApiManager().setMerchantManagementSigningKeyId(getFileHelper().getValueFromPropertiesFile(
                        Hooks.envProperties, "developer-signing-key-id-pos"));

                testContext.getApiManager().setMerchantManagementSigningKey(getFileHelper().getValueFromPropertiesFile(
                        Hooks.envProperties, "developer-signing-key-pos"));
            }
        }
        catch (NullPointerException e){
            logger.info("********* Hitting Sandbox APIM ****************");
            ...
            testContext.getApiManager().getAccessToken().setType("sandbox");
            testContext.getApiManager().getAccessToken().setMerchantDetails(getFileHelper().getValueFromPropertiesFile(Hooks.envProperties, "developer-client-id-pod"),
                    getFileHelper().getValueFromPropertiesFile(Hooks.envProperties, "developer-client-secret-pos"));
